...
def auditoria_total(row, tipo='saida'):
    """
    Motor de Auditoria: Cruza CFOP, CST e Valores.
    ...
    # --- AUDITORIA ICMS PRÓPRIO ---
    if cfop == '6403' and vlr_icms == 0:
        erros.append("ICMS Próprio zerado no CFOP 6403.")
        cliente.append("Destacar ICMS Próprio na NF-e (Substituto Tributário).")
        dominio.append("Configurar Acumulador para calcular ICMS Próprio em ST (Substituto).")
    ...
    if tipo == 'saida' and vlr_icms > 0 and bc_icms > 0:
        ...
        if abs(calc - vlr_icms) > 0.05:
            ...
            cliente.append("Corrigir cálculo de ICMS no faturamento.")
            dominio.append("Verificar alíquota no cadastro do produto.")

    # --- AUDITORIA ICMS ST ---
    if cst in cst_st and vlr_st == 0:
        erros.append(f"CST {cst_full} exige ICMS ST, valor zerado.")
        cliente.append("Informar valor do ICMS ST retido.")
        dominio.append("No acumulador, aba Estadual, marcar 'Gera guia de ST'.")
    elif vlr_st > 0 and cst not in cst_st and cst != '60':
        erros.append(f"ST indevido para CST {cst_full}.")
    ...
    if cfop in ['5101', '6101'] and vlr_ipi == 0:
        ...
        cliente.append("Destacar IPI (Saída de Produção Própria).")
    ...
    if tipo == 'saida' and cfop.startswith('6'):
        ...
        if uf_dest in reg_7 and aliq not in [7.0, 4.0]:
            erros.append(f"Alíquota UF {uf_dest} incorreta (7%).")
            cliente.append(f"Ajustar alíquota para 7% p/ {uf_dest}.")

    return pd.Series({
        'DIAGNOSTICO_ERRO': " | ".join(erros) if erros else "Escrituração Regular",
        'PARAMETRO_CLIENTE': " | ".join(cliente) if cliente else "-",
        'SOLUÇÃO_CONTABIL': " | ".join(dominio) if dominio else "-"
    })

def main():
    st.title("⚖️ Curador: Auditoria Integral ICMS / ST / IPI")
    st.markdown("---")
    
    # Upload Centralizado (Sem Sidebar)
    c1, c2 = st.columns(2)
    ...
    if ent_f and sai_f:
        try:
            ...
            # Aplicação das Auditorias e Correções
            df_ent[['DIAGNOSTICO_ERRO', 'PARAMETRO_CLIENTE', 'SOLUÇÃO_CONTABIL']] = df_ent.apply(lambda r: auditoria_total(r, 'entrada'), axis=1)
            df_sai[['DIAGNOSTICO_ERRO', 'PARAMETRO_CLIENTE', 'SOLUÇÃO_CONTABIL']] = df_sai.apply(lambda r: auditoria_total(r, 'saida'), axis=1)
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                pd.DataFrame([{'ICMS': v_icms, 'ST': v_st, 'IPI': v_ipi}]).to_excel(writer, sheet_name='Apuração de Saldos', index=False)
                ...
                for sheet, df_ref in [('Entradas Auditadas', df_ent), ('Saídas Auditadas', df_sai)]:
                    ...
                    for i, val in enumerate(df_ref['DIAGNOSTICO_ERRO']):
                        if val != "Escrituração Regular": ws.set_row(i + 1, None, fmt_red)

            st.download_button("📥 Baixar Auditoria do Curador", output.getvalue(), "Auditoria_Curador_Resultado.xlsx")
